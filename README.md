...
通配符 `*` 可以在路径中使用，表示匹配当前层级的所有元素（列表中的每个元素或字典中的每个值）。支持多层嵌套通配符。
...
5. **自动设置默认节点** - 对于没有显式配置 `default` 的 selector，会自动测试其叶节点子节点到代理服务器的 TCP 连接延迟，并将延迟最小的节点设置为 `default`。如果所有测试都失败，则保持 `default` 未设置。
